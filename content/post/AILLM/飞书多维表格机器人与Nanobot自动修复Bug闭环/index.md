...
这不是一条“全 AI 工作流”，而是一套 **Human-in-the-loop（人在回路）** 的混合工作流。固定规则交给自动化，高变化任务交给 Agent，高风险决策仍然由人完成。
...
- **Workflow（工作流）**：大模型和工具沿预定义路径执行。
- **Agent（智能体）**：大模型根据执行结果，动态决定下一步行动和工具调用。
- **Deterministic Automation（确定性自动化）**：相同输入按照固定规则进入相同路径，例如表格触发器、Webhook 和消息路由。
